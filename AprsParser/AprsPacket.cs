...
namespace aprsparser
{
  ...
  public class AprsPacket
  {
    ...
    public TimeInfo TimeStamp { get; set; }
    ...
    private string ParsePositionAndSymbol(string ps)
    {
      try
      {
        ...
        else
        {
          ...
          //strip off position report and return remainder of string
          //Course and speed
          if(char.IsDigit(ps[19])) //if it's digit, then it should be course/speed
          {
            string[] sarCSESPD = ps.Substring(19, 7).Split('/');
            int iCourse, iSpeed;
            if(int.TryParse(sarCSESPD[0], out iCourse))
              Position.Course = iCourse;

            if(int.TryParse(sarCSESPD[1], out iSpeed))
              Position.Speed = iSpeed;
          }

          //Altitude
          if (ps.Substring(19).Contains("/A="))
          {
            int iAltitudeStartIndex = ps.IndexOf('=');
            int iAltitude = 0;
            if(int.TryParse(ps.Substring(iAltitudeStartIndex + 1, 6), out iAltitude))
              Position.Altitude = iAltitude;

            ps = ps.Substring(iAltitudeStartIndex + 7);
          }
          else
          {
            ps = ps.Substring(19);
